...
const CLUES: Clue[] = [
  ...
  { 
    ...
    color: '#118ab2', 
    rus: 'Экран телефона, светящийся в темноте, показывает входящее сообщение. Текст, написанный заглавными буквами, гласит: «Если ты не отдашь мне контракт завтра, ты пожалеешь об этом». Прочитав это, становится ясно, что Наташе угрожали.', 
    vocab: [
  ...
  const getWordDefinition = async (word: string) => {
    ...
    try {
      ...
      if (!apiKey || apiKey === 'MY_GEMINI_API_KEY' || apiKey === '') {
        throw new Error("API-ключ не настроен. Пожалуйста, добавьте GEMINI_API_KEY в настройки Vercel.");
      }

      const response = await ai.models.generateContent({
        model: "gemini-3-flash-preview",
        contents: `Объясни значение слова "${cleanWord}" на простом русском языке для студента уровня B1. 
        Если это причастие или деепричастие, обязательно укажи это и объясни, от какого глагола оно образовано.
        Дай краткое и понятное определение (1-2 предложения).`,
      });
      setAiDefinition(response.text || "Не удалось найти определение.");
    } catch (error: any) {
      console.error("AI Error:", error);
      let errorMsg = "Ошибка при получении определения.";
      if (error.message?.includes("API-ключ")) {
        errorMsg = error.message;
      } else if (error.message?.includes("API key")) {
        errorMsg = "Неверный или отсутствующий API-ключ. Проверьте настройки Vercel.";
      }
      setAiDefinition(errorMsg);
    } finally {
      setIsDefining(false);
    }
  };
                ...
                {definingWord && (
                  ...
                  >
                    <div className="bg-[#1a1025] border-2 border-[#ffd166] p-6 rounded-2xl relative">
                      ...
                      {isDefining ? (
                        <div className="flex items-center gap-3 text-[#a990c8] animate-pulse">
                          <div className="w-4 h-4 bg-[#ffd166] rounded-full animate-bounce" />
                          Ищем определение...
                        </div>
                      ) : (
                        <div className="space-y-4">
